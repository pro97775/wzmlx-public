...
from pyrogram.errors import FloodWait, PhotoInvalidDimensions, RPCError
...
class TelegramUploader:
    ...
    async def _upload_progress(self, current, total, file_path):
        if self._listener.is_cancelled:
            raise StopTransmission()
        self._file_progress[file_path] = current
        self._processed_bytes = sum(self._file_progress.values())

    async def _get_thumb_and_meta(self, file_path, force_document, user_thumb):
        ...
        return key, thumb, duration, width, height, artist, title
    ...
    async def _send_with_retry(self, send_func, **kwargs):
        while True:
            try:
                ...
                LOGGER.warning(f"FloodWait {f.value}s while uploading")
                await sleep(f.value + 1)

    async def _try_send(self, key, client, kwargs):
        senders = {
            "videos": client.send_video,
            "audios": client.send_audio,
            "photos": client.send_photo,
        }
        sender = senders.get(key, client.send_document)
        try:
    ...
    async def _upload_file(
        ...
        try:
            (
                ...
                artist,
                title,
            ) = await self._get_thumb_and_meta(o_path, force_document, user_thumb)
            ...
            if self._listener.up_dest:
                ...
                upload_chat_id, thread_id = self._sent_msg.chat.id, None

            reply_to_message_id = (
                self._sent_msg.id if upload_chat_id == self._sent_msg.chat.id else None
            )
